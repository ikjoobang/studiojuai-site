		...
		init : function(pid) { 
			...
			*/
			...
			$(function() {
				...
				$(`[form-type="radio.image"] input, [form-type="check.image"] input`).on('change', function() {
                    let $formEL = $(this).closest('.element[data-type="form"]');
                    $formEL.find('input[type="checkbox"], input[type="radio"]').each(function(idx, v){
                        let $label = $(v).closest('label');
                        if($(v).is(':checked')) {
