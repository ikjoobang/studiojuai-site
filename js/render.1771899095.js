...
var RENDER = {
    init: function (property) {
							...
							success: function (data) {
								...
								showPageCallback(showPage, function() {
									...
									setTimeout(function() {
										...
										// 원페이지, 갤러리 상세페이지에서 메뉴 클릭 시 페이지 이동 후 해당 메뉴로 이동하지 않고 상단 고정 Issue Fix (https://flow.team/l/1x7rX)
										if(typeof b.SID != 'undefined' && b.SID && b.SID == 'ronixinc') {
											if(location.hash) {
												var hash = decodeURIComponent(location.hash.substring(1));  
												console.log('해시 감지:', hash);
												
												setTimeout(function() {
													var $block = $('.link-to-' + hash);
													...
													if($block.length > 0) {
														...
														if(Math.abs(blockTop - currentScroll) > threshold) {
															console.log('스크롤 필요:', {current: currentScroll, target: blockTop});
															moveLinkPage(hash);
														} else {
															console.log('이미 근처에 있음, 스크롤 생략');
														}
													} else {
														console.error('블록 없음:', '.link-to-' + hash);
													}
												}, 500);
											}
										}
		...
		var moveLinkPage = function(link,interval,isblock) {
			...
			if((clCheckBrowser('safari') && clCheckBrowser('mobile')) || $('.' + link).hasClass('el_0')) duration = 0;
		...
		var loadPush = function(data) {
				...
				if(val.type == 'gallery') {
					...
					if($glmBtnWrap.length == 0) {
						...
						if(checkLoadmore) {
							...
							if($tag.find('.gallery-loadmore[data-id="'+val.seq+'"]').length > 0) $('.gallery-loadmore[data-id="'+val.seq+'"]').replaceWith(glm_btn);
					...
					//갤러리블럭 모바일 가로 갯수 설정 시 클래스추가
					var gal_css = CSSJSON.toJSON(htmlspecialchars_decode(val.elcss)),
						gal_s = style.get(gal_css,val.elname),
						gal_case = $tag.attr('data-case'),
						gal_id = $tag.attr('data-id'),
						gal_name = $tag.attr('data-name');
					if(gal_s.__layout_m_on == 1 && gal_case != 'gallery61') {
						$tag.addClass('el-col-m'+gal_s.__layout_col_mc);
					} else if(gal_s.__layout_m_on == 1 && gal_case == 'gallery61') {
						$tag.addClass('el-col-m'+gal_s.__swiper_view_mc);
						setTimeout(function() {
							elGalleryJS[gal_id].gjs_load(document.querySelector('.'+gal_name));
						});
					}
				}
			...
			dataloop(data).then(function() {
				...
				// if($('.element[data-parallax="true"]').length > 0) setParallax();
		        if($('.element[data-parallax="true"]').length > 0) setParallaxGsap();
		...
		function setDefaultForm(el) {
			...
			$(el).find('.checkbox-inline').removeClass('checked');
    		$(el).find('.radio-inline').removeClass('checked');
		...
		$('[data-form-submit]').live('click', function() { 
			...
			$.each($items, function(i,v) {
				...
				switch(type) {
					...
					case "radio.image":
					case "check.image":
						if(type == 'radio' || type == 'radio.image') {
			...
			if(!$parent.hasClass('el-fixedcontact') && $('.form-inline .error').length > 0) {
				...
				$.each($error, function(i,v) {
					if(i==0) {
						...
						if($.inArray(type, ['radio', 'check', 'radio.image', 'check.image']) > -1 || privacy) {
			...
			if(submit) {
		        ...
		        $.ajax({
					...
					data: { sid : b.SID, pid : pid, values : values, check : check, captcha : formCaptcha, block_type: 'form' },
					dataType: 'json',
					type: 'POST',
		            async: true,
		            cache: false,
		            ...
		            success: function (data) {
		                ...
		                $.each(data.list,function(i,v) {
							...
							if(checkGallerySVG) {
								...
								gimgSVG.find('image').attr('xlink:href', src);
								gimgSVG.find('polygon, path').attr('fill', 'url(#gimgSvg_'+val.elname+'_'+item_index+')');
							}
...
``
				...
				if(b.VALIDPLAN && b.VALIDTYPE != 'PK') {
					...
					if(b.VALIDTYPE == 'BN' || b.VALIDTYPE == 'SM') {
						...
						if(typeof b.SETTINGS.qm_onoff != 'undefined' && b.SETTINGS.qm_onoff==true) {	
							...
							if(!(b.PARENT.page != null && (quick_type=='mobile'))) {
								if($('.menu-lock-block').length>0) return false;
				...
				if(property.ISLOCK == 'false') {
					if(property.COUNT==0) {
						...
						if(checkUrl[0] == "forum" && b.VIEW) {				
							...
							var ps_fmview = $.forum.view(b.VIEW);
							ps_fmview.done(function(data) {
								if($(`.page-comments[data-id="${b.VIEW}"]`).length == 0) _this.displayComment(data);
			...
			isMenuLock(function() {
				...
				$.each(contents, function(i,v) {
					...
					if(c.type == 'forum') {
						...
						//갤러리블럭 모바일 가로 갯수 설정 시 클래스추가
						var gal_css = CSSJSON.toJSON(htmlspecialchars_decode(c.elcss)),
							gal_s = style.get(gal_css,c.elname),
							gal_case = $el.attr('data-case');
						if(gal_s.__layout_m_on == 1 && gal_case != 'gallery61') {
							$el.addClass('el-col-m'+gal_s.__layout_col_mc);
						} else if(gal_s.__layout_m_on == 1 && gal_case == 'gallery61') {
							$el.addClass('el-col-m'+gal_s.__swiper_view_mc);
						}
					} else if(c.type == 'sync') {
	            ...
	            // if($('.element[data-parallax="true"]').length > 0 && $('.element[data-parallax-type="scroll"]').length > 0) setParallax();
		        if($('.element[data-parallax="true"]').length > 0) setParallaxGsap();
		...
		if( i.length>0 || (i.length==0 && is_gc_cookie) ) {
			...
			$.each(g_data.list,function(index,v) {
				...
				if(checkGallerySVG) {
					...
					gimgSVG.find('image').attr('xlink:href', src);
					gimgSVG.find('polygon, path').attr('fill', 'url(#gimgSvg_'+el.elname+'_'+index+')');					
				}
	...
	displayComment : function(viewData) {
		if (typeof viewData == 'undefined' || !viewData) viewData = {};
		var checkUrl = this.b.PAGE.split(',');
		var type = (checkUrl[0] == 'forum') ? 'F' : 'P';
		var check_view_empty = (this.b.VIEW && this.b.COUNT == 1 && $('.error_404.element[data-name="error_404"]').length == 1) ? true : false;
		if(type == "P" 
			&& (this.b.COUNT == 0 || check_view_empty)
		) {
	    ...
	    if(type == 'F') {
			if($.isEmptyObject(viewData) && this.b.PARENT.settings) viewData = {elsettings: jQuery.parseJSON(this.b.PARENT.settings)};
			let tmp_elsettings = getOrDefault(viewData,'elsettings',{});
	    	let tmp_fmsettings = getOrDefault(viewData,'fm_settings',{});

			let comment_display = 'ON';
		    ...
		    if(comment_display == 'OFF') return false;
	    } else {
...
var fonts_ko = ["@Korean", "IBM Plex Sans KR", "KoPub Batang", "Black Han Sans", "Gmarket Sans", "Godo", "Kukdetopokki", "Nanum Gothic", "Nanum Gothic Coding", "Nanum Myeongjo", "Nanum Barun Gothic", "Nanum Pen Script", "Nanum Barun Pen", "Nanum Brush Script", "Nanum Square", "Nanum Square Round", "Daraehand", "Dohyeon", "Recipekorea", "Monsori", "Noto Sans KR", "Noto Serif CJK KR", "BB Tree Gothic","BB Tree Namu",  "BB Tree Hand", "Seoul Namsan", "Seoul Hangang", "Suit", "Spoqa Han Sans", "S CoreDream", "Yeonsung", "Oseong and HanEum", "Iropke Batang", "EASTARJET", "Jalnan", "Jeju Gothic", "Jeju Myeongjo", "Jeju Hallasan", "Jua", "Youth", "Pretendard", "Hangyule", "Hanna"],
    ...
    fonts_ch = ["@Chinese", "cwTeXHei", "cwTeXKai", "cwTeXMing", "cwTeXYen", "Noto Sans SC", "Noto Sans TC", "Noto Serif SC", "Noto Serif TC"],
