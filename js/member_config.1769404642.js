...
$(function() {
	...
	$(document).off('click', '.mng-settings[data-type="collection"] .collection-item-used, .mng-settings[data-type="collection"] .collection-item-optional')
			   .on('click', '.mng-settings[data-type="collection"] .collection-item-used, .mng-settings[data-type="collection"] .collection-item-optional', function() {
		if($(this).closest('.checkboxs').hasClass('disabled')) {
			var modal = $(this).showModalFlat($.lang[LANG]['config.information'], $.lang[LANG]['manager.check.collection.required.message'], true, false, '', 'ok','','cl-cmmodal cl-s-btn w560 cl-p130 cl-p0');
			return false;
		}
		...
		if (check_class === 'used' && cl_val.toString() === 'false' && $(this).closest('.use-option-check').find('.collection-item-optional').prop('checked') === true) {
			jcol_data[0]['require'] = 'false';
			$(this).closest('.use-option-check').find('.collection-item-optional').prop('checked', false);
		}
		if (check_class === 'require' && cl_val.toString() === 'true' && $(this).closest('.use-option-check').find('.collection-item-used').prop('checked') === false) {
			jcol_data[0]['used'] = 'true';
			$(this).closest('.use-option-check').find('.collection-item-used').prop('checked', true);
		}
